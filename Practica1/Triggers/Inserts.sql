...
BEGIN 
    ...
    DECLARE @Mensaje AS VARCHAR(MAX);
    SET @Mensaje = 'Se ha insertado un nuevo curso con id: ' + CAST(@CodCourse AS VARCHAR(36)) + ', y nombre: ' + @Name;
...
BEGIN 
    ...
    DECLARE @Mensaje AS VARCHAR(MAX);
    SET @Mensaje = 'Se ha insertado una nueva asignación de curso al estudiante: ' + CAST(@StudentId AS VARCHAR(36)) + ', al curso: ' + CAST(@CourseCodCourse AS VARCHAR(36));
...
BEGIN 
    ...
    DECLARE @CourseCodCourse AS int;
    SET @CourseCodCourse = CONVERT(int, (SELECT CodCourse FROM INSERTED));

    DECLARE @Mensaje AS VARCHAR(MAX);
    SET @Mensaje = 'Se ha insertado una nueva asignación de tutor al curso: ' + CAST(@CourseCodCourse AS VARCHAR(36)) + ', el tutor será: ' + CAST(@TutorId AS VARCHAR(36));
...
BEGIN 
    ...
    DECLARE @Mensaje AS VARCHAR(MAX);
    SET @Mensaje = 'Se ha insertado un nuevo perfil de usuario al estudiante: ' + CAST(@UserId AS VARCHAR(36)) + ', con: ' + CAST(@Credits AS VARCHAR(36)) + ' créditos';
